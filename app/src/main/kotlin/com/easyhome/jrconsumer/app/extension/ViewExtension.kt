...
import com.easyhome.jrconsumer.app.imageloader.JRImageConfig
import com.easyhome.jrconsumer.R
import com.easyhome.jrconsumer.api.Api
import com.easyhome.jrconsumer.app.listener.SingleClickListener
...
fun ImageView.loadImage(url: String, placeholder: Int) {
    ArmsUtils.obtainAppComponentFromContext(this.context)
        .imageLoader().loadImage(
            this.context, JRImageConfig.Builder()
...
fun getImageView(context: Activity, button: Button, imageView: ImageView) {
    if (Util.isOnMainThread()) {
        Glide.with(context)
            .asDrawable()
            .load("${Api.BASE}/dynamic/addSub?uuid=${UserInfoManager.getInstance().uuid}")
            .apply(getRequest())
