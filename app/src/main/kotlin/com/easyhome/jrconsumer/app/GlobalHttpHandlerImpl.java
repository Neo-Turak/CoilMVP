...
public class GlobalHttpHandlerImpl implements GlobalHttpHandler {
    ...
    public Request onHttpRequestBefore(Interceptor.Chain chain, Request request) {
        /* 如果需要再请求服务器之前做一些操作, 则重新返回一个做过操作的的 Request 如增加 Header, 不做操作则直接返回参数 request8*/
        if ("POST".equals(chain.request().method())) {
            String versionName = DeviceUtils.getVersionName(context);
            String deviceCode = DeviceUtils.getUniquePsuedoID(context);
            HttpUrl newUrl = request.url().newBuilder(request.url()+"").build();
            ...
            HttpUrl httpUrl = request.url().newBuilder()
                    .addQueryParameter("versionNew", versionName)
                    .addQueryParameter("appTypeNew", "C")
                    .addQueryParameter("osTypeNew", "android")
                    .addQueryParameter("deviceCodeNew", deviceCode)
                    .build();
