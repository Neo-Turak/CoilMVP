...
public class PermissionUtil {
    ...
    public static void requestPermission(final RequestPermission requestPermission, RxPermissions rxPermissions, String... permissions) {
        ...
        if (needRequest.isEmpty()) {//全部权限都已经申请过，直接执行操作
            ...
            rxPermissions
                    .requestEach(needRequest.toArray(new String[needRequest.size()]))
                    .buffer(permissions.length)
                    .subscribe(new ResourceObserver<List<Permission>>() {
                        @Override
     ...
     * 申请权限
     */
    public static void mRequestPermission(RequestPermission requestPermission, RxPermissions rxPermissions, RxErrorHandler errorHandler, String... permissions) {
        requestPermission(requestPermission, rxPermissions, errorHandler, permissions);
    }
